...
public class StringUtils {
	...
	private static String capitalizeFirstChars(String string) {
		if (null != string) {
			...
			if (string.toLowerCase().equals("id")) {
				return string.toUpperCase(java.util.Locale.getAvailableLocales()[0]);
			} else if (null != (words = string.split(" "))) {
				if (words.length > 0) {
					switch (words.length) {
					case 1:
	...
	public static boolean validInteger(String num) {
		Pattern p = Pattern.compile("^\\d+$");
		Matcher m = p.matcher(num);
		return m.find();
	}
	
	public static boolean validInteger(int num) {
		Pattern p = Pattern.compile("^\\d+$");
